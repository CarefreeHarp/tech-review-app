...
<head>
    ...
    <style>
        ...
        #diagram svg.is-focused .flowchart-link:not(.is-related) { opacity: 0.035; }
        #diagram svg.is-focused .edgeLabel:not(.is-related) { opacity: 0.02; }
        ...
        #diagram svg.is-focused .edgeLabel.is-related div { padding: 3px 6px; background: #fde047; border: 2px solid #ca8a04; border-radius: 5px; box-shadow: 0 2px 5px rgba(15, 23, 42, 0.45); }
        #diagram svg.is-focused .edgeLabel.is-related span { color: #111827 !important; font-size: 15px; font-weight: 800; opacity: 1 !important; }
    </style>
</head>
<body>
    <header>
...
${diagramSvg}
    ...
    <script>
        ...
        // Mermaid emits edge labels in the same declaration order as their paths.
        const labelToLink = new Map(labels.map((label, index) => [label, links[index]]));
        ...
        const setLabelVisibility = (label, isRelated) => {
            const container = label.querySelector("div");
            const text = label.querySelector("span");
            const opacity = isRelated ? "1" : "0.02";

            label.style.setProperty("opacity", opacity, "important");
            label.querySelector("foreignObject")?.style.setProperty("opacity", opacity, "important");

            if (isRelated) {
                container?.style.setProperty("background", "#fde047", "important");
                container?.style.setProperty("border", "2px solid #ca8a04", "important");
                container?.style.setProperty("border-radius", "5px", "important");
                container?.style.setProperty("padding", "3px 6px", "important");
                container?.style.setProperty("box-shadow", "0 2px 5px rgba(15, 23, 42, 0.45)", "important");
                ...
                ["background", "border", "border-radius", "padding", "box-shadow"].forEach((property) => container?.style.removeProperty(property));
                ["color", "fill", "font-weight", "opacity"].forEach((property) => text?.style.removeProperty(property));
            }
        };

        const clearFocus = () => {
            diagram.classList.remove("is-focused");
            [...nodes, ...links].forEach((element) => element.classList.remove("is-related"));
            labels.forEach((label) => {
                label.classList.remove("is-related");
                label.style.removeProperty("opacity");
                label.querySelector("foreignObject")?.style.removeProperty("opacity");
                const container = label.querySelector("div");
                const text = label.querySelector("span");
                ["background", "border", "border-radius", "padding", "box-shadow"].forEach((property) => container?.style.removeProperty(property));
                ["color", "fill", "font-weight", "opacity"].forEach((property) => text?.style.removeProperty(property));
        ...
        const focusNode = (nodeId) => {
            ...
            labels.forEach((label) => {
                const isRelated = relatedLinks.includes(labelToLink.get(label));
                label.classList.toggle("is-related", isRelated);
                setLabelVisibility(label, isRelated);
            });
        ...
        const initialNodeId = new URLSearchParams(window.location.search).get("focus");
        if (nodes.some((node) => node.dataset.id === initialNodeId)) focusNode(initialNodeId);
    </script>
</body>
</html>`;
